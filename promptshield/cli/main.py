...
def scan(
    ...
    context: Optional[str] = typer.Option(None, "--context", "-c", help="System prompt or context"),
    pretty: bool = typer.Option(False, "--pretty", "-p", help="Human-readable colored output"),
    api_key: Optional[str] = typer.Option(None, "--api-key", help="Provider API key override"),
    model: Optional[str] = typer.Option(None, "--model", help="Provider LLM model override"),
):
    ...
    if pretty:
        ...
        color = typer.colors.GREEN if result.verdict == "safe" else typer.colors.RED
        icon = "✅" if result.verdict == "safe" else "🛡️"
        verdict_str = typer.style(result.verdict.upper() + f" {icon}", fg=color, bold=True)
    ...
    if result.verdict == "blocked" or result.verdict == "review":
...
def init():
    ...
    if config_path.exists():
        typer.secho("Configuration file .promptshield.yaml already exists.", fg=typer.colors.YELLOW)
...
def server():
    ...
    config = ShieldConfig.load()
    typer.secho(f"Starting PromptShield Server on {config.server.host}:{config.server.port}...", fg=typer.colors.BLUE)
    uvicorn.run("promptshield.server.app:app", host=config.server.host, port=config.server.port, reload=False)
