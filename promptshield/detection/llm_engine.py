...
async def scan_llm(prompt: str, config: ShieldConfig, context: Optional[str] = None) -> Tuple[str, float, str]:
    """Returns (verdict, confidence, threat_type) via Provider API"""
    if not config.provider.api_key:
        return "review", 0.5, "none"
    ...
    system_prompt = (
        ...
        "Respond ONLY in JSON format: {'verdict': 'safe'|'blocked'|'review', 'confidence': 0.0-1.0, "
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            return result.get("verdict", "review"), float(result.get("confidence", 0.5)), result.get("threat_type", "none")
