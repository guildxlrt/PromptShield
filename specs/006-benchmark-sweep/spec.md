...
**Independent Test**: Run `promptshield-benchmark sweep --thresholds "0.40,0.60"` and verify that (a) three benchmark runs execute, (b) a ranked table is printed to stdout, and (c) `benchmarks/sweep_results.json` is written with exactly three entries in `results`.
...
- **FR-004**: Default thresholds when `--thresholds` is not specified: `0.40`, `0.60`.
