...
- [x] T009 Add `get_runtime_config()` function to `benchmarks/runner.py`. The function returns a dict with keys `promptshield_llm_model`, `promptshield_embedding_model`, and `confidence_threshold`, reading from environment variables with documented fallback values (`"unknown"` or `"0.6"`).
