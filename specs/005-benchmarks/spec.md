...
- **Unset config variables**: If environment variables are not set, the benchmark does not fail — `get_runtime_config()` returns sentinel values (`"unknown"` or `"0.6"`) and the user is expected to check the startup console output or the JSON to identify the actual config.
